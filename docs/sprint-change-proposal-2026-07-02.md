...
2. **Pin target: JUMP to v0.22.0** (`860f4268299739ce9df87c4f373abe35824101cf`, latest upstream release) — overriding the forensics' repair-at-0.15.0-first recommendation. Verification performed before ruling ripples: upstream v0.22.0 **still lacks** the Global.lua nil guards (only the `#args<2` early-return is safe), so `0001` was **regenerated against v0.22.0** (apply-check PASS, sentinel ×3, LuaJIT compile OK); `0002`/`0003` apply clean on **both** tags. Ripples applied: story 3.5.2 retargeted (title/ACs/tasks + jump caveat + fallback lane at `3e1b71c9`), story 3.5.5 capture protocol → GUI v0.22.0, plan amendments at E3.5 item 2 / E4 items 1 & 3. **Known ripple: v0.22.0 ships `TreeData/0_5/`** — the Epic 4 tree bump and Epic 5 artifacts target 0_5, not 0_4.
   **OUTCOME (same day):** story 3.5.2 was executed immediately; the v0.22.0 jump **failed the test gate** — MinimalCalc cannot boot v0.22.0's `CalcSetup` API (`:875 env.spec:CollectGrantedPassiveNodesFromItems` + granted-nodes/weapon-slot contracts; canary 18/18 F, integration 119F/125P) — and **AC5's pre-committed fallback was executed: active pin = v0.15.0 `3e1b71c9`** (real submodule; tree content-identical to pre-repair backup, 0-entry EOL-controlled diff; suites green per story Change Log). The jump is **deferred to the Epic 4 spike**, which deletes MinimalCalc: v0.22.0 objects already live in the submodule history, and the v0.22.0 patch variant is parked at `docs/forensics/proposed-patches/0001-global-lua-nil-safety-v0220.patch`. Story 3.5.5 captures at the **active** pin.
