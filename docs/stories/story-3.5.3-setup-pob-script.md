...
- Relevant architecture patterns and constraints
  ...
  - As of 2026-07-02 `external/patches/` holds the promoted set of three: `0001` Global.lua (regenerated for the v0.22.0 pin decided the same day), `0002` ModStore.lua, `0003` CalcOffence.lua, plus `.gitattributes` (`*.patch -text`); the stale 0.12.2-cut `global-lua-nil-safety.patch` is retired (`stub_functions.lua` was found absent from the tree — not adopted) — the script must handle N patches without modification
