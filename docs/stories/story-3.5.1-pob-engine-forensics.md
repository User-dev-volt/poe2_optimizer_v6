...
- Draft patches stay under `docs/forensics/proposed-patches/` as the immutable forensic record (v0.15.0-targeted). **Update 2026-07-02: promotion happened** — Alec approved and `external/patches/` now holds `0001..0003` (0001 regenerated for the newly decided v0.22.0 pin; stale 0.12.2 patch retired). This story's remaining job is ratification of the inventory, not promotion.
